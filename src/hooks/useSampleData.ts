...
export const useSampleData = () => {
  ...
  const checkAndCreateSampleData = async () => {
    ...
    try {
      // Check if user already has experiences
      ...
      // If no experiences, create sample data
      if (!existingExperiences || existingExperiences.length === 0) {
        await createSampleExperiences();
      }
    } catch (error) {
  ...
  const createSampleExperiences = async () => {
    if (!user) return;

    try {
      // Create sample experiences
      const experiencesToCreate = sampleExperiences.map(exp => ({
        ...exp,
        user_id: user.id,
        is_published: true
      }));
      ...
      const { error } = await supabase
        .from('experiences')
        .insert(experiencesToCreate);

      if (error) {
        console.error('Error creating sample experiences:', error);
        return;
      }

      // Add some likes to make it more realistic
      const { data: createdExperiences } = await supabase
        .from('experiences')
        .select('id')
        .eq('user_id', user.id);

      if (createdExperiences) {
        const likeCounts = [34, 89, 156, 203];
        const commentCounts = [12, 23, 45, 67];

        for (let i = 0; i < createdExperiences.length; i++) {
          await supabase
            .from('experiences')
            .update({ 
              likes_count: likeCounts[i] || 0,
              comments_count: commentCounts[i] || 0 
            })
            .eq('id', createdExperiences[i].id);
        }
      }

      toast({
        title: "Benvenuto in LifeShare!",
        description: "Abbiamo creato alcune esperienze di esempio per iniziare.",
      });
